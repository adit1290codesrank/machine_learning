...
Matrix BatchNorm::backward_pass(const Matrix& delta,double learning_rate)
{
    ...
    #pragma omp parallel for
    for(int i=0;i<delta.rows;i++)for(int j=0;j<features;j++)prev_delta(i,j)=(g(0,j)*std_inv(0,j)/delta.rows)*(delta.rows*delta(i,j)-db(0,j)-x_(0,j)*std_inv(0,j)*dg(0,j));
    t++;
    ...
    for(int i=0;i<features;i++)
    {
        ...
        mg(0,i)=b1*mg(0,i)+(1.0-b1)*mg(0,i);
        vg(0,i)=b2*vg(0,i)+(1.0-b2)*vg(0,i)*vg(0,i);
